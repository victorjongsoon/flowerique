...
function addItem() {
	...
	$stmt = $conn->prepare($qry); 
	$stmt->bind_param("ii", $_SESSION["Cart"], $pid); // "i" - integer // $_SESSION["Cart"] = $row["ShopCartID"]; 
	$stmt->execute(); 
...
function removeItem() {
	...
	$stmt = $conn->prepare($qry); 
	$stmt->bind_param("ii", $_POST["product_id"], $_SESSION["Cart"]); //"i" - integer 
	$stmt->execute(); 
	unset($_SESSION["NumCartItem"]); 
	$shopCartItems = $stmt->get_result();

	if($shopCartItems->num_rows >0){
		foreach($shopCartItems as $cartItems)
		{
			$_SESSION["NumCartItem"] += 1; 
		}
	}
